...
% ---- 3.1 Thành viên 1 ----
\section{Thành viên 1 -- AI tạo podcast: \texttt{ai-service} và \texttt{content-service}}
\label{sec:phase3-ai-content}

\subsection{Phạm vi phụ trách}

Thành viên 1 phụ trách luồng creator tạo podcast/show bằng AI và phần lưu trữ nội dung nghe được trong Content Service. Phạm vi bao gồm hai service chính:

\begin{itemize}[leftmargin=2cm]
  \item \textbf{\texttt{ai-service}:} Quản lý voice profile, chat-create thread, chat message, production plan, generation job, script generation, TTS, transcript alignment và điều phối tạo show/episode.
  \item \textbf{\texttt{content-service}:} Quản lý show, episode, host, episode assets, transcript segments, home feed, show detail, episode detail và bookmark episode.
\end{itemize}

\subsection{Chức năng chính}

\begin{enumerate}[leftmargin=2cm]
  \item Creator tạo thread trò chuyện với AI để mô tả ý tưởng podcast.
  \item Creator gửi message để refine ý tưởng, tone, đối tượng nghe, format và số episode.
  \item AI tạo production plan gồm series title, mô tả, host, episode drafts, tag và nguồn tham khảo.
  \item Creator xác nhận tạo show từ production plan.
  \item Background worker tạo show, host, script, audio, transcript, episode và assets.
  \item Người dùng xem home feed, show detail, episode detail và bookmark episode.
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Use case chi tiết cụm AI và Content}
\label{tab:uc-ai-content}
\renewcommand{\arraystretch}{1.3}
\small
\begin{tabularx}{\textwidth}{|c|l|l|X|}
\hline
...
\hline
AI-UC-01 & Tạo chat thread & Creator & Creator mở màn hình AI Setup, hệ thống tạo thread mới để lưu lịch sử trao đổi. \\
\hline
AI-UC-02 & Gửi message refine ý tưởng & Creator & Creator gửi yêu cầu, \texttt{ai-service} gọi Gemini để sinh phản hồi và cập nhật hướng sản xuất. \\
\hline
AI-UC-03 & Xem production plan & Creator & Creator xem kế hoạch gồm title, description, hosts, episode drafts, tone và tags. \\
\hline
AI-UC-04 & Tạo show từ plan & Creator, Hệ thống AI & Creator xác nhận plan, hệ thống tạo job nền để sinh show/episode/audio/transcript. \\
\hline
AI-UC-05 & Xem show/episode & User & Người dùng xem chi tiết show, danh sách episode và thông tin phát audio. \\
\hline
AI-UC-06 & Bookmark episode & User & Người dùng lưu hoặc bỏ lưu episode yêu thích vào thư viện cá nhân. \\
\hline
\end{tabularx}
\end{table}
...
Hình~\ref{fig:component-ai-content} mô tả các component chính trong luồng AI tạo podcast và luồng đọc/nghe nội dung.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{diagrams/component_ai_content.pdf}
  \caption{Component diagram cụm \texttt{ai-service} và \texttt{content-service}}
  \label{fig:component-ai-content}
\end{figure}

\subsection{Mô hình dữ liệu riêng}

AI DB lưu dữ liệu phục vụ quá trình tạo nội dung: \texttt{voice\_profiles}, \texttt{chat\_threads}, \texttt{chat\_messages}, \texttt{production\_plans}, \texttt{production\_plan\_hosts}, \texttt{production\_plan\_episode\_drafts} và \texttt{generation\_jobs}. Content DB lưu dữ liệu nghe được sau khi tạo: \texttt{shows}, \texttt{episodes}, \texttt{show\_hosts}, \texttt{episode\_segments}, \texttt{episode\_assets} và \texttt{episode\_bookmarks}. Giữa hai database không có foreign key vật lý; \texttt{ai-service} ghi sang Content DB thông qua repository/contract nội bộ.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{diagrams/er_ai_content.pdf}
  \caption{ER riêng cho cụm AI tạo podcast và Content}
  \label{fig:er-ai-content}
\end{figure}

\subsection{Sequence diagram}

Hình~\ref{fig:seq-ai-create-show} mô tả luồng creator tạo show từ production plan. Điểm quan trọng là API trả về \texttt{job\_id} trước, còn các bước sinh script, TTS, transcript và ghi episode chạy bất đồng bộ trong worker.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{diagrams/seq_ai_create_show.pdf}
  \caption{Sequence diagram tạo show bằng AI}
  \label{fig:seq-ai-create-show}
\end{figure}

Hình~\ref{fig:seq-content-listen} mô tả luồng người dùng mở episode để nghe audio và xem transcript.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{diagrams/seq_content_listen.pdf}
  \caption{Sequence diagram nghe episode và xem transcript}
  \label{fig:seq-content-listen}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Endpoint chính cụm AI và Content}
\label{tab:endpoint-ai-content}
\renewcommand{\arraystretch}{1.3}
\scriptsize
\begin{tabularx}{\textwidth}{|l|l|X|}
\hline
...
\hline
POST & \texttt{/api/v1/ai/chat-create/threads} & Tạo thread chat AI mới cho creator. \\
\hline
POST & \texttt{/api/v1/ai/chat-create/threads/\{thread\_id\}/messages} & Gửi message, nhận phản hồi AI và có thể sinh production plan. \\
\hline
GET & \texttt{/api/v1/ai/production-plans/\{plan\_id\}} & Lấy chi tiết production plan. \\
\hline
POST & \texttt{/api/v1/ai/production-plans/\{plan\_id\}/create-show} & Tạo job nền để sinh show/episode từ plan. \\
\hline
GET & \texttt{/api/v1/public/content/home} & Lấy home feed public. \\
\hline
GET & \texttt{/api/v1/public/content/shows/\{showID\}} & Lấy chi tiết show. \\
\hline
GET & \texttt{/api/v1/public/content/episodes/\{episodeID\}} & Lấy chi tiết episode, audio URL và transcript. \\
\hline
POST & \texttt{/api/v1/content/bookmarks} & Bookmark episode cho user đã đăng nhập. \\
\hline
\end{tabularx}
\end{table}
...
Các màn hình liên quan gồm: màn hình tạo podcast/AI setup, màn hình chat tạo plan, màn hình My Shows/Library, màn hình show detail và màn hình player/transcript. Về kết quả, cụm service đã có cấu trúc dữ liệu cho voice profiles, production plans, generation jobs, show/episode và bookmark; các endpoint chính phục vụ luồng creator và listener đã được tách qua API Gateway.

\subsection{Hạn chế riêng}

Luồng AI phụ thuộc API key, quota và độ ổn định của provider bên thứ ba. TTS, transcript alignment và upload audio cần chạy bất đồng bộ nên cần cơ chế theo dõi job tốt hơn nếu triển khai production.

% ---- 3.2 Thành viên 2 ----
\section{Thành viên 2 -- Báo và podcast báo: \texttt{article\_service} và \texttt{embedding\_service}}
\label{sec:phase3-article-embedding}

\subsection{Phạm vi phụ trách}

Thành viên 2 phụ trách luồng đọc báo, crawl bài báo, category, tương tác bài viết, podcast từ nhóm bài báo, embedding và semantic search. Hai service chính là:

\begin{itemize}[leftmargin=2cm]
  \item \textbf{\texttt{article\_service}:} Crawl RSS, quản lý bài báo, category, reaction, comment, metric đọc bài và article podcast job.
  \item \textbf{\texttt{embedding\_service}:} Consume CDC event từ Kafka, chunk nội dung, gọi embedding provider, lưu vector bằng pgvector, semantic search và gán category ngữ nghĩa.
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Use case chi tiết cụm Article và Embedding}
\label{tab:uc-article-embedding}
\renewcommand{\arraystretch}{1.3}
\small
\begin{tabularx}{\textwidth}{|c|l|l|X|}
\hline
\textbf{Mã} & \textbf{Use case} & \textbf{Tác nhân} & \textbf{Mô tả} \\
\hline
AR-UC-01 & Xem danh sách bài báo & User & Lấy danh sách bài báo, có phân trang và lọc theo category/query. \\
\hline
AR-UC-02 & Xem chi tiết bài báo & User & Lấy nội dung bài báo, tăng view count và trả reaction/comment summary. \\
\hline
AR-UC-03 & Reaction/comment bài báo & User & Người dùng tương tác với bài báo, service lưu interaction/comment và cập nhật thống kê. \\
\hline
AR-UC-04 & Tạo podcast từ bài báo & User & Người dùng chọn nhiều bài báo, tạo job nền để tổng hợp script, audio và transcript. \\
\hline
AR-UC-05 & Tạo embedding khi có bài mới & Hệ thống & Debezium phát event, embedding service consume và tạo vector cho article. \\
\hline
AR-UC-06 & Tìm kiếm semantic & User & User nhập query, service embedding tìm các bài liên quan theo độ tương đồng vector. \\
\hline
\end{tabularx}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{diagrams/component_article_embedding.pdf}
  \caption{Component diagram cụm \texttt{article\_service} và \texttt{embedding\_service}}
  \label{fig:component-article-embedding}
\end{figure}

\subsection{Mô hình dữ liệu riêng}

Article DB quản lý bài báo và tương tác qua các bảng \texttt{articles}, \texttt{categories}, \texttt{category\_articles}, \texttt{article\_stats}, \texttt{article\_interactions}, \texttt{article\_comments}, \texttt{article\_metrics}, \texttt{article\_podcast\_jobs}, \texttt{article\_podcast\_job\_articles}, \texttt{article\_podcast\_drafts} và \texttt{article\_podcast\_assets}. Embedding DB lưu document/chunk/vector/job và kết quả category match.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{diagrams/er_article_embedding.pdf}
  \caption{ER riêng cho cụm Article và Embedding}
  \label{fig:er-article-embedding}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{diagrams/seq_article_detail.pdf}
  \caption{Sequence diagram xem chi tiết bài báo}
  \label{fig:seq-article-detail}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{diagrams/seq_article_podcast.pdf}
  \caption{Sequence diagram tạo podcast từ nhóm bài báo}
  \label{fig:seq-article-podcast}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Endpoint chính cụm Article và Embedding}
\label{tab:endpoint-article-embedding}
\renewcommand{\arraystretch}{1.3}
\scriptsize
\begin{tabularx}{\textwidth}{|l|l|X|}
\hline
\textbf{Method} & \textbf{Endpoint} & \textbf{Mục đích} \\
\hline
GET & \texttt{/api/v1/article} & Lấy danh sách bài báo. \\
\hline
GET & \texttt{/api/v1/article/categories} & Lấy danh sách category bài báo. \\
\hline
GET & \texttt{/api/v1/article/\{article\_id\}} & Lấy chi tiết bài báo. \\
\hline
POST & \texttt{/api/v1/article/\{article\_id\}/reactions} & Ghi reaction của user. \\
\hline
GET & \texttt{/api/v1/article/\{article\_id\}/comments} & Lấy danh sách comment. \\
\hline
POST & \texttt{/api/v1/article/podcast-jobs} & Tạo job podcast từ nhóm bài báo. \\
\hline
GET & \texttt{/api/v1/article/podcast-jobs/\{job\_id\}} & Theo dõi trạng thái job podcast. \\
\hline
POST & \texttt{/api/v1/public/embedding/articles/search} & Tìm kiếm semantic theo query. \\
\hline
\end{tabularx}
\end{table}
...
Các giao diện liên quan gồm news list, article detail, chọn bài để tạo podcast, article podcast job/library/playback và kết quả semantic search. Cụm service đã tách rõ dữ liệu bài báo với dữ liệu embedding; Debezium/Kafka giúp phát hiện bài mới để tạo vector và đồng bộ category ngữ nghĩa về Article DB.

\subsection{Hạn chế riêng}

Chất lượng search và category matching phụ thuộc chất lượng nội dung crawl, prompt/chunking và embedding model. Podcast từ bài báo cần xử lý nền vì các bước research, script, TTS và upload audio có độ trễ cao.

% ---- 3.3 Thành viên 3 ----
\section{Thành viên 3 -- Hạ tầng và xác thực: \texttt{api-gateway} và \texttt{identity-service}}
\label{sec:phase3-identity-gateway}

\subsection{Phạm vi phụ trách}

Thành viên 3 phụ trách cửa vào hệ thống và danh tính người dùng: API Gateway, routing public/protected, JWT middleware, sign-up/sign-in, Google sign-in, refresh token, sign-out, email verification, forgot/reset password, profile \texttt{me} và outbox event gửi sang Notification Service.
...
% ---- 3.4 Thành viên 4 ----
\section{Thành viên 4 -- Thông báo: \texttt{notification-service}}
\label{sec:phase3-notification}

\subsection{Phạm vi phụ trách}

Thành viên 4 phụ trách notification inbox, unread count, mark read/read all, notification settings, internal API tạo notification, gửi email xác minh/reset password, consume Kafka event, retry/DLQ, processed-event tracking và delivery logs.
...
Bảng~\ref{tab:phase3-output} tổng hợp các đầu ra của Phase 3 theo từng cụm phụ trách.

\begin{table}[H]
\centering
\caption{Tổng hợp đầu ra Phase 3}
...
\hline
AI + Content & Component, ER, sequence tạo show, sequence nghe episode & Chat-create, production plan, generation job, show/episode, player/transcript, bookmark. \\
\hline
Article + Embedding & Component, ER, sequence article detail, sequence article podcast & Crawl/read article, reaction/comment/metric, article podcast job, CDC embedding, semantic search. \\
\hline
Gateway + Identity & Component, ER, sequence sign-up/verify, sequence protected route & Routing public/protected, JWT, sign-up/sign-in, Google sign-in, refresh, reset password, outbox event. \\
\hline
...
Chương này tổng hợp kết quả triển khai hệ thống Pody, cách cài đặt và chạy demo, các test case kiểm thử chính, số liệu minh chứng từ dữ liệu seed/demo và đánh giá hạn chế của hệ thống.
...
\begin{table}[H]
\centering
\caption{Kết quả theo cụm service phụ trách}
\label{tab:ket-qua-ca-nhan}
\renewcommand{\arraystretch}{1.3}
\scriptsize
\begin{tabularx}{\textwidth}{|l|X|X|}
\hline
\textbf{Cụm service} & \textbf{Chức năng đã hoàn thành} & \textbf{Minh chứng chính} \\
\hline
AI + Content & Chat-create, production plan, job tạo show, ghi show/episode, home feed, detail, player transcript, bookmark. & API AI/Content, bảng AI DB/Content DB, màn hình create, show detail, player. \\
\hline
Article + Embedding & Crawl/seed article, list/detail article, category, reaction/comment/metric, article podcast job, CDC embedding, semantic search. & API Article/Embedding, Debezium/Kafka flow, bảng Article DB/Embedding DB. \\
\hline
Gateway + Identity & Sign-up, sign-in, Google sign-in, refresh token, forgot/reset password, profile, JWT auth tại Gateway. & Route metadata, Identity DB, outbox event, màn hình auth/profile. \\
\hline
Notification & Inbox, unread count, mark read/read all, settings, email verification/reset, internal notification, retry/DLQ contract. & Notification DB, delivery log, Kafka consumer, màn hình notifications/settings. \\
\hline
\end{tabularx}
\end{table}
...
\hline
Identity docs & \texttt{http://localhost:8080/api/v1/public/identity/docs} \\
\hline
AI docs & \texttt{http://localhost:8080/api/v1/public/ai/docs} \\
\hline
Content docs & \texttt{http://localhost:8080/api/v1/public/content/docs} \\
\hline
Notification docs & \texttt{http://localhost:8080/api/v1/public/notifications/docs} \\
\hline
...
Các test case dưới đây được dùng để kiểm tra những luồng quan trọng nhất của hệ thống. Khi demo thực tế, nhóm có thể bổ sung thêm cột ảnh chụp màn hình hoặc response JSON minh chứng.
...
\hline
\endhead
Auth & Đăng ký tài khoản mới & Tạo user, tạo verification token và publish email event. & Tạo user và ghi outbox event trong Identity DB. & Pass \\
\hline
...
\hline
AI & Tạo show từ production plan & Tạo generation job, sau đó ghi show/episode sang Content DB. & Job chạy nền, kết quả hiển thị ở My Shows/Home. & Pass \\
\hline
...
Bảng~\ref{tab:so-lieu-demo} ghi nhận số liệu demo dựa trên seed/migration trong repository và các luồng có thể phát sinh thêm dữ liệu khi demo. Với các bảng phụ thuộc AI provider hoặc thao tác demo trực tiếp, giá trị thực tế cần được chốt lại sau khi chạy Docker Compose và truy vấn database.

\begin{table}[H]
\centering
\caption{Số liệu demo và nguồn minh chứng}
...
\hline
Notifications/delivery logs & Chốt khi demo & Phát sinh từ seed inbox, auth email event hoặc internal notification. \\
\hline
\end{tabularx}
\end{table}
...
\begin{itemize}[leftmargin=2cm]
  ...
  \item Các luồng AI, TTS và embedding phụ thuộc API key, quota, latency và chất lượng phản hồi của nhà cung cấp bên thứ ba.
  \item Một số job nền như tạo audio, transcript alignment, article podcast và embedding cần cơ chế quan sát trạng thái chi tiết hơn cho người dùng cuối.
  ...
  \item Push notification mobile, billing/subscription, moderation và recommendation cá nhân hóa nâng cao chưa nằm trong phạm vi hoàn thiện của bài tập lớn.
\end{itemize}
...
\begin{enumerate}[leftmargin=2cm]
  ...
  \item Hoàn thiện billing/subscription nếu thương mại hóa tính năng tạo podcast bằng AI.
\end{enumerate}
...
\begin{enumerate}[leftmargin=2cm]
  ...
  \item Tài liệu nội bộ repository: README của \texttt{api-gateway}, \texttt{identity-service}, \texttt{content-service}, \texttt{ai-service}, \texttt{article\_service}, \texttt{embedding\_service} và \texttt{notification-service}.
\end{enumerate}
...
\noindent
\textit{Kết thúc Phase 4 -- Kết quả, thử nghiệm và đánh giá.}
