...
class FakeRepository:
    def __init__(self, document: ArticleEmbeddingDocument | None) -> None:
        ...
        self.deleted_category_matches: list[int] = []
        self.deleted_category_matches_with_sync: list[dict[str, object]] = []
        self.refreshed_category_matches: list[dict[str, object]] = []
    ...
    def delete_article_category_matches(self, *, article_id: int) -> None:
        self.deleted_category_matches.append(article_id)

    def delete_article_category_matches_and_enqueue_sync_event(self, **kwargs) -> None:
        self.deleted_category_matches_with_sync.append(kwargs)

    def refresh_article_category_matches_for_article(self, **kwargs) -> None:
        self.refreshed_category_matches.append(kwargs)
...
class ArticleEmbeddingServiceTests(unittest.TestCase):
    ...
    def test_chunking_signature_change_forces_reembed(self):
        ...
        self.assertEqual(repository.replaced_embeddings[0]["max_matches"], 3)
        self.assertEqual(repository.replaced_embeddings[0]["min_score"], 0.2)
        self.assertEqual(
            len(repository.replaced_embeddings[0]["document_embedding"]),
            self.settings.gemini.output_dimensions,
        )
    ...
    def test_non_public_article_is_skipped_without_calling_provider(self):
        ...
        self.assertEqual(
            repository.deleted_category_matches_with_sync,
            [
                {
                    "article_id": 42,
