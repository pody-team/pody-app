...
\begin{titlepage}
  ...
  \begin{center}
    ...
    \begin{tabular}{r l}
      ...
      \textbf{Thành viên thực hiện:} & \textit{Vũ Đức Thành -- B22DCCN801} \\[6pt]
    \end{tabular}
...
\section{Vũ Đức Thành -- Báo và podcast báo: \texttt{article\_service} và \texttt{embedding\_service}}
\label{sec:phase3-article-embedding}

\subsection{Phạm vi phụ trách}

Vũ Đức Thành phụ trách luồng đọc báo, crawl bài báo, category, tương tác bài viết, podcast từ nhóm bài báo, embedding và semantic search. Hai service chính là:
...
\begin{table}[H]
\centering
\caption{Tổng hợp đầu ra Phase 3 -- Article + Embedding}
...
Chương này tách rõ kết quả chung của nhóm và kết quả riêng của phần cá nhân: Vũ Đức Thành -- Báo và podcast báo. Phần triển khai local được giữ ở mức tổng quát, còn bảng kết quả, kiểm thử và số liệu minh chứng tập trung vào cụm Article + Embedding.
...
\begin{table}[H]
\centering
\caption{Kết quả cá nhân -- Article + Embedding}
...
\hline
Cụm phụ trách & Article + Embedding \\
\hline
Chức năng đã hoàn thành & Crawl/seed article, list/detail article, category, reaction/comment/metric, article podcast job, CDC embedding, category matching và semantic search. \\
\hline
Minh chứng chính & API Article/Embedding, Debezium/Kafka flow, Article DB, Embedding DB, màn hình news list/article detail. \\
\hline
\end{tabularx}
\end{table}
...
\caption{Bảng test case cá nhân -- Article + Embedding}
\label{tab:test-case}\\
\hline
...
\begin{table}[H]
\centering
\caption{Số liệu demo cá nhân -- Article + Embedding}
...
\begin{itemize}[leftmargin=2cm]
  \item Chất lượng search phụ thuộc chất lượng nội dung crawl, chunking và embedding model.
  \item Article podcast cần xử lý nền vì research, script, TTS và upload audio có độ trễ cao.
  \item CDC/category sync cần theo dõi idempotency và retry khi Kafka hoặc provider lỗi.
\end{itemize}
...
\noindent
\textit{Kết thúc bản báo cáo cá nhân -- Vũ Đức Thành -- Báo và podcast báo.}
